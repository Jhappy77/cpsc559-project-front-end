        ...
        <Flex backgroundImage='linear-gradient(to bottom right, green, yellow)' width='100%' height='calc(100vh)' alignItems='center' textAlign='center' justifyContent='center'>
            <Flex alignItems='center' justifyContent="center">
                <VStack>
                    ...
                    <Button leftIcon={<FaArrowLeft />} onClick={() => navigate("/")}>Back</Button>
                    <form onSubmit={(e) => {onSubmit(e);}}>
                        <Flex alignItems="center" display={displayForm}>
                            <Input fontWeight={"bold"} variant="filled" value={code} onChange={handleChange} maxLength={CODE_LENGTH} border="2px" focusBorderColor="yellow" placeholder="Enter code here" _placeholder={{ color: 'white' }}  size={["md", "lg"]} colorScheme="whiteAlpha" />
                            <Button backgroundColor="white" type="submit" color="black" _hover={{backgroundColor: "", color: "black"}} m={4}>Submit</Button>
                        </Flex>
                    </form>
                    <VStack display={displayLoading} width="100%" >
                        <Text fontFamily={`'Open Sans', sans-serif`} fontSize="2xl">Waiting for host to start the game...</Text>
